...
var someOtherDouble: Double = someDouble
...
// TODO: Create a variable called 'age' and set it equal to a number. Then create three boolean variables called: 'canDrink', 'canVote', and 'canDrive'. Create a series of 'if', 'else if' and 'else' statemeprints to set them appropriately. Print out the results after. Change around the number to ensure that your logic is working correctly:
...
// TODO: Set the below stri`ng, multilingual, to the concatenation (+) of text and moreText
...
var characterCount: Int = multilingual.characters.count
...
struct TestRunner {
    func runTests(testClass:AnyClass) {
        let tests = testClass as! XCTestCase.Type        let testSuite = tests.defaultTestSuite()

        testSuite.run()         let run = testSuite.testRun as! XCTestSuiteRun;
